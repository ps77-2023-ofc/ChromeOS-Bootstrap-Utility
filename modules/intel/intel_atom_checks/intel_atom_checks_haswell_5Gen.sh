...
echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    cat << "Q131-1"
...
if [ "$haswell_cros_version" == "1" ]; then
    ...
    wget -q --show-progress "https://github.com/sebanc/brunch/releases/download/r86-k5.4-stable-20201201/brunch_r86_k5.4_stable_20201201.tar.gz" -O brunch.tar.gz
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13421.99.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    ...
    mv chromeos_13421.99.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "2" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r87"
    echo "VERSIÓN DE CHROME OS: v87"
    echo "CODENAME DEL RECOVERY DESTINO: rammus"
    echo "Descargando archivos necesarios..."
    wget -q --show-progress "https://github.com/sebanc/brunch/releases/download/r87-stable-20201227/brunch_r87_stable_20201227.tar.gz" -O brunch.tar.gz
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13505.111.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_13505.111.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "3" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r88"
    echo "VERSIÓN DE CHROME OS: v88"
    echo "CODENAME DEL RECOVERY DESTINO: rammus"
    echo "Descargando archivos necesarios..."
    wget -q --show-progress "https://github.com/sebanc/brunch/releases/download/r88-stable-20210131/brunch_r88_stable_20210131.tar.gz" -O brunch.tar.gz
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13729.56.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_13729.56.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "4" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r89"
    echo "VERSIÓN DE CHROME OS: v89"
    echo "CODENAME DEL RECOVERY DESTINO: rammus"
    echo "Descargando archivos necesarios..."
    wget -q --show-progress "https://github.com/sebanc/brunch/releases/download/r89-stable-20210403/brunch_r89_stable_20210403.tar.gz" -O brunch.tar.gz
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13729.56.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    ...
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "5" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    ...
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13816.82.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_13816.82.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Archivo del recovery renombrado." 
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "6" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    ...
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13904.55.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_13904.55.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Archivo del recovery renombrado."
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "7" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
...
Q14-1
# Leer la respuesta del usuario
read -n 1 haswell_brunch_version_93_cros_version
# Limpiar la terminal
clear
# Si la versión escogida es 1 (Chrome OS v92), comenzar la descarga de los archivos necesarios
if [ "$haswell_brunch_version_93_cros_version" == "1" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r93"
    ...
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_brunch_version_93_cros_version" == "2" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r93"
...
elif [ "$haswell_cros_version" == "8" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    ...
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_14150.74.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_14150.74.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Archivo del recovery renombrado."
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_cros_version" == "9" ]; then 
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
...
Q14-2
# Leer la respuesta del usuario
read -n 1 haswell_brunch_version_97_cros_version
# Limpiar la terminal
clear
# Si la versión escogida es 1 (Chrome OS v96), comenzar la descarga de los archivos necesarios
if [ "$haswell_brunch_version_97_cros_version" == "1" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r97"
    ...
    wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_14268.67.0_rammus_recovery_stable-channel_mp-v2.bin.zip" -O recovery.zip
    echo "Archivos descargados."
    echo "Extrayendo archivos..."
    tar -xzf brunch.tar.gz
    unzip -qq recovery.zip
    echo "Archivos extraídos."
    echo "Renombrando archivo del recovery a recovery.bin..."
    mv chromeos_14268.67.0_rammus_recovery_stable-channel_mp-v2.bin recovery.bin
    echo "Archivo del recovery renombrado."
    echo "Comenzando la construcción de la imagen..."
    bash "./chromeos-install.sh -src recovery.bin -dst chromeos.img"
elif [ "$haswell_brunch_version_97_cros_version" == "2" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
    echo "VERSIÓN DE BRUNCH FRAMEWORK: r97"
...
elif [ "$haswell_cros_version" == "10" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Haswell (5ª Gen)"
