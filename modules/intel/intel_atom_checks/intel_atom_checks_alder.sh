...
echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
    cat << "Q131-7"
...
if [ "$alder_lake_cros_version" == "1" ]; then
...
elif [ "$alder_lake_cros_version" == "2" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
    ...
    echo "Comenzando la construcción de la imagen..."
    bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
elif [ "$alder_lake_cros_version" == "3" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
...
Q14-1
# Leer la respuesta del usuario
read -n 1 alder_brunch_version_93_cros_version
# Limpiar la terminal
clear
# Si la versión escogida es 1 (Chrome OS v92), comenzar la descarga de los archivos necesarios
    if [ "$alder_brunch_version_93_cros_version" == "1" ]; then
        echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
        echo "VERSIÓN DE BRUNCH FRAMEWORK: r93"
        ...
        wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_13982.88.0_volteer_recovery_stable-channel_mp-v6.bin.zip" -O recovery.zip -P /cros_workfolder
        echo "Archivos descargados."
        echo "Extrayendo archivos..."
        tar -xzf brunch.tar.gz -C cros_workfolder
        unzip -qq recovery.zip -d cros_workfolder
        echo "Archivos extraídos."
        echo "Renombrando archivo del recovery a recovery.bin..."
        mv cros_workfolder/chromeos_13982.88.0_volteer_recovery_stable-channel_mp-v6.bin cros_workfolder/recovery.bin
        echo "Archivo del recovery renombrado."
        echo "Comenzando la construcción de la imagen..."
        bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
    elif [ "$alder_brunch_version_93_cros_version" == "2" ]; then
        echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
        echo "VERSIÓN DE BRUNCH FRAMEWORK: r93"
        ...
        echo "Descargando archivos necesarios..."
        wget -q --show-progress "https://github.com/sebanc/brunch/releases/download/r93-stable-20211002/brunch_r93_stable_20211002.tar.gz" -O brunch.tar.gz -P /cros_workfolder
        wget -q --show-progress "https://web.archive.org/web/20220302163230mp_/https://dl.google.com/dl/edgedl/chromeos/recovery/chromeos_14092.77.0_volteer_recovery_stable-channel_mp-v8.bin.zip" -O recovery.zip -P /cros_workfolder
        echo "Archivos descargados."
        echo "Extrayendo archivos..."
        tar -xzf brunch.tar.gz -C cros_workfolder
        unzip -qq recovery.zip -d cros_workfolder
        echo "Archivos extraídos."
        echo "Renombrando archivo del recovery a recovery.bin..."
        mv cros_workfolder/chromeos_14092.77.0_volteer_recovery_stable-channel_mp-v8.bin cros_workfolder/recovery.bin
        echo "Archivo del recovery renombrado."
        echo "Comenzando la construcción de la imagen..."
        bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
    fi
elif [ "$alder_lake_cros_version" == "4" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
    ...
    echo "Comenzando la construcción de la imagen..."
    bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
elif [ "$alder_lake_cros_version" == "5" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
...
Q14-2
# Leer la respuesta del usuario
read -n 1 alder_brunch_version_100_cros_version
# Limpiar la terminal
clear
    if [ "$alder_brunch_version_100_cros_version" == "1" ]; then
        echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
        echo "VERSIÓN DE BRUNCH FRAMEWORK: r100"
        ...
        echo "Comenzando la construcción de la imagen..."
        bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
    elif [ "$alder_brunch_version_100_cros_version" == "2" ]; then
        echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
        echo "VERSIÓN DE BRUNCH FRAMEWORK: r100"
        ...
        echo "Comenzando la construcción de la imagen..."
        bash "./cros_workfolder/chromeos-install.sh -src cros_workfolder/recovery.bin -dst output/chromeos.img"
    elif [ "$alder_brunch_version_100_cros_version" == "3" ]; then
        echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
        echo "VERSIÓN DE BRUNCH FRAMEWORK: r100"
...
elif [ "$alder_lake_cros_version" == "6" ]; then
    echo "PROCESADOR OBJETIVO: Intel Atom, plataforma Alder Lake (13ª Gen)"
